...
- [x] Manual Postman check of retry / replay / 422 / 400 (see `segment7.md` §8) — passed 2026-09-25; Postgres rows `c1`/`p1` COMPLETED. **Open:** Redis showed no `idem:*` keys after the run, cache write is silently failing or pointing at a different Redis (Segment 6 follow-up, not blocking)
